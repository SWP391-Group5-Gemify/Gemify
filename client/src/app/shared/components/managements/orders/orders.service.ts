...
export class OrdersService {
  ...
  getOrders() : Observable<PaginationModel<Order>>{
    ...
    if(this.orderParams.orderTypeId > 0) params = params.append('orderTypeId', this.orderParams.orderTypeId);
    params = params.append('pageIndex', this.orderParams.pageIndex);
    params = params.append('pageSize', this.orderParams.pageSize);
    if(this.orderParams.search) params = params.append('search', this.orderParams.search);

    return this.http.get<PaginationModel<Order>>(this.baseUrl + '/orders', {params});
  }
